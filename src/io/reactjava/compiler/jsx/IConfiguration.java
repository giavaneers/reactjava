...
import io.reactjava.compiler.jsx.IJSXTransform.INativeComponent;
import java.io.File;
...
@name       getNativeComponent - get native component for specified tag name
                                                                              */
                                                                             /**
            Get native component for specified tag name.

@return     native component for specified tag name

@param      tagName     native component tag name
...
@notes
                                                                              */
//------------------------------------------------------------------------------
static INativeComponent getNativeComponent(
   String     tagName,
   TreeLogger logger)
{
   logger.log(
      logger.INFO,
      "jsx.IConfiguration.getNativeComponent(): entered for tagName="+tagName);

   INativeComponent component = () -> tagName;

   String componentPath =
      IConfiguration.getNativeComponentJavascript(tagName, logger);
   ...
   if (componentPath != null)
   {
      logger.log(
         logger.INFO,
         "jsx.IConfiguration.getNativeComponent(): adding dependency="
   ...
   logger.log(
      logger.INFO,
      "jsx.IConfiguration.getNativeComponent():"
    + " exiting, componentPath=" + componentPath
    + ", component=" + component);
...
@name       getNativeComponentJavascript - get library component javascript
                                                                              */
                                                                             /**
...
@notes
                                                                              */
//------------------------------------------------------------------------------
public static String getNativeComponentJavascript(
   String     tagName,
   TreeLogger logger)
{
   ...
   if (componentPath == null)
   {
      componentPath = getDependencies().get(tagName);
   }
   if (componentPath == null)
   {
   ...
   TreeLogger logger)
{
   if (logger != null)
   {
      logger.log(
         logger.INFO,
         "jsx.IConfiguration.getNodeModuleJavascript(): entered"
       + " for module=" + module);
   }
   ...
   if (module.contains("/"))
   {
      ...
      File candidate = new File(getNodeModulesDir(logger), module);
      if (candidate.exists())
      {
         nodeModulePath = candidate.getAbsolutePath();
      }
   }
   if (nodeModulePath == null)
   {
                                       // check for implicit path             //
      String relPath = module.replace(".", "/");

      File candidate = new File(getNodeModulesDir(logger), relPath + ".js");
      if (!candidate.exists())
      {
                                       // a directory                         //
         candidate = new File(getNodeModulesDir(logger), relPath);
         if (candidate.exists())
         {
            candidate = new File(candidate, candidate.getName() + ".js");
         }
      }
      if (candidate.exists())
      {
   ...
   {
      nodeModulePath =
         getNodeModuleJavascriptFromNodeModuleDirectory(
            module,
            getNodeModulesDir(logger),
            module,
            logger);
   }
//importedNodeModulesStart//
// if (nodeModulePath == null && IPreprocessor.isAppComponent(refType))
//   {
//      nodeModulePath =
//         getNodeModuleJavascriptFromAppDeclaration(module, refType, logger);
//   }
   ...
   if (logger != null)
   {
      logger.log(
         logger.INFO,
         "jsx.IConfiguration.getNodeModuleJavascript(): exiting"
       + " for module=" + module
       + " with nodeModulePath=" + nodeModulePath);
   }
...
@name       getNodeModuleJavascriptFromAppDeclaration - node module javascript
                                                                              */
                                                                             /**
            Get node module javascript path from app declaration.

@return     node module javascript path from app declaration
...
@notes
                                                                              */
//------------------------------------------------------------------------------
//static String getNodeModuleJavascriptFromAppDeclaration(
//   String     module,
//   TypeDsc    refType,
//   TreeLogger logger)
//{
...
//   String nodeModulePath = null;
//   String classname      = ((ComponentDsc)refType).getClassname();
//   logger.log(
//      logger.INFO,
//      "jsx.IConfiguration.getNodeModuleJavascriptFromAppDeclaration():"
//    + " checking for importedNodeModules declared by " + classname);
//   try
//   {
//      Collection<String> importedNodeModules =
//         AppComponentInspector.newInstance(
//            refType,
//            IPreprocessor.getParsedProvidersAndComponents(logger), logger)
//               .getParsedImportedNodeModules(logger);
//
//      for (String imported : importedNodeModules)
//      {
//         String[] splits = imported.split(" as ");
//         if (splits.length > 1 && splits[1].trim().equals(module))
//         {
//            nodeModulePath = getNodeModuleJavascript(splits[0].trim(), logger);
//            break;
//         }
//      }
//   }
...
//   logger.log(
//      logger.INFO,
//      "jsx.IConfiguration.getNodeModuleJavascriptFromAppDeclaration(): exiting"
//    + " for module=" + module
//    + " with nodeModulePath=" + nodeModulePath
...
@name       getNodeModuleJavascriptFromNodeModuleDirectory - get javascript
                                                                              */
                                                                             /**
            Get node module javascript path from node module directory.

@return     node module javascript from node module directory
...
@notes
                                                                              */
//------------------------------------------------------------------------------
static String getNodeModuleJavascriptFromNodeModuleDirectory(
   String     module,
   File       nodeModules,
   String     filename,
   TreeLogger logger)
{
   ...
   logger.log(
      logger.INFO,
      "jsx.IConfiguration.getNodeModuleJavascriptFromNodeModuleDirectory():"
    + " module=" + module
    + ", moduleDir=" + moduleDir
    + ", filename="  + filename);
   ...
   logger.log(
      logger.INFO,
      "jsx.IConfiguration.getNodeModuleJavascriptFromNodeModuleDirectory():"
    + " exiting for module=" + module
    + " with nodeModulePath=" + nodeModulePath
    + ", elapsed msce=" + (System.currentTimeMillis() - start));
...
@name       getWProjectIMLFile - get project module file
                                                                              */
                                                                             /**
            Get project module file.

@return     project module file or null if not found
...
@notes
                                                                              */
//------------------------------------------------------------------------------
static File getProjectIMLFile(
   TreeLogger logger)
{
   File projectIMLFile = null;
   for (File chase : getProjectDirectory(null, logger).listFiles())
   {
      if (chase.getName().endsWith(".iml"))
      {
         projectIMLFile = chase;
         break;
      }
   }

   return(projectIMLFile);
}
/*------------------------------------------------------------------------------
