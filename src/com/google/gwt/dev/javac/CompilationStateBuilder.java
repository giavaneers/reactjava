...
public class CompilationStateBuilder {
  ...
  public class CompileMoreLater {
     ...
     * unit will also be serialized to the core-unitcache file. (As a result, each AST will be
...
/*LBM-START*/
                                       ...
                                       // zip files                           //
    Map<String,String> candidates = new java.util.HashMap<>();
    ...
    for (Resource resource : resources)
    {
       ...
       logger.log(
           logger.DEBUG,
           "CompilationStateBuilder.doBuildFrom(): "
         + " checking resource=" + url);
       ...
       if (idx == 0)
       {
          ...
          {
                                       // during general build...             //
             String delimiter = "/out/production/";
             idx = url.indexOf(delimiter);
             if (idx > 0)
             {
                idx = url.indexOf('/', idx + delimiter.length());
             }
             else
             {
                                       // during run/debug...                 //
       ...
       if (idx >= 0)
       {
         ...
         for (int i = 0; bSupported && i < filterExclude.length; i++)
         {
            bSupported = !classname.startsWith(filterExclude[i]);
         }
         if (bSupported)
         {
            try
            {
              ...
              logger.log(
                 logger.DEBUG,
                 "CompilationStateBuilder.doBuildFrom(): "
               + " added component candidate with classname=" + classname);
            }
            catch(Exception e)
            {
    ...
    for (Resource resource : resources) {
      ...
      if (resource instanceof com.google.gwt.dev.resource.impl.FileResource)
      {
         resource = resource;
      }
      //CompilationUnitBuilder builder = CompilationUnitBuilder.create(resource);
      CompilationUnitBuilder builder =
         CompilationUnitBuilder.create(resource, candidates, logger);
/*LBM-END*/
  ...
  private ContentId getResourceContentId(Resource resource) {
    ...
    try {
      ...
      // TODO: deprecate com.google.core.dev.util.Util and use Guava.
      Util.copy(in, out);
    } catch (IOException e) {
